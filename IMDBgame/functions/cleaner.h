#define full_file_size 100000 //изменить !
#define clean_file_size 50000 // изменить!

int get_file_size(FILE* f)
{
	fseek(f, 0, SEEK_END);
	size = ftell(f)/(2*sizeof(int));
	return (size);
}
...
void write_clean_array(int actor_to_actor_arr[][2], int size, FILE* actors_file)
{
	...
	{
		if ((actor_to_actor_arr[i] != actor_to_actor_arr[i - 1]) && (actor_to_actor_arr[i][0] != actor_to_actor_arr[i][1]))
		{
...
int main(void)
{
	actor_to_actor_arr[full_file_size][2];
	int size = get_file_size(actors_file);

	make_array(actor_to_actor_arr, actors_file, size);

	write_clean_array(actor_to_actor_arr, size, actors_file);

	size = get_file_size(actors_file);
}
