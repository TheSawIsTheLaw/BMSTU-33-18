...
//
int search_start_index_for_film(int films_actors[][COUNT_OF_ELEMENTS_IN_ONE_CONECTION], int cur_film)
{
    int index = bin_search(cur_film, films_actors, COUNT_OF_CONECTIONS_FILMS_ACTOR);
...
void make_array_actor_to_actor(int actors_films[][COUNT_OF_ELEMENTS_IN_ONE_CONECTION], int films_actors[][COUNT_OF_ELEMENTS_IN_ONE_CONECTION], FILE *actors_file)
{
    int n;
    FILE *cur_file;
    int position = 0;
    int prev_actor = actors_films[0][ONE];
    int cur_actor = -1;
    int cur_film = -1;
    n = 0;
    ...
    for (int el = ZERO; el < COUNT_OF_CONECTIONS_ACTOR_FILMS; el++)
    {
        ...
        cur_film = actors_films[el][ZERO];


        int index_for_film = search_start_index_for_film(films_actors, cur_film);

        while (films_actors[index_for_film][ZERO] == cur_film)
        {
            if (films_actors[index_for_film][ONE] != cur_actor)
            {

                if (check_unic(cur_file, films_actors[index_for_film][ONE], n))
                {
                    n += 1;
                    fwrite(&films_actors[index_for_film][ONE], sizeof(int), 1, cur_file);
                }
                ++index_for_film;
             }
            else
